...
void animacion() {
    const std::string mensaje = ":D se le aprecia mucho, <3 unos....... 20 puntitos ? :D";
    const std::vector<std::string> colores = {RED, GREEN, YELLOW, BLUE, MAGENTA, CYAN, BRIGHT_RED, BRIGHT_GREEN, BRIGHT_YELLOW, BRIGHT_BLUE, BRIGHT_MAGENTA, BRIGHT_CYAN, BRIGHT_WHITE};
    const int duracion = 5000; // Duración de la animación en milisegundos
    const int intervalo = 500; // Intervalo entre cada cambio de color en milisegundos
    const int iteraciones = duracion / intervalo;

    for (int i = 0; i < iteraciones; ++i) {
        system("CLS"); // Limpiar la pantalla
        std::cout << colores[i % colores.size()] << mensaje << RESET << std::endl;
        std::this_thread::sleep_for(std::chrono::milliseconds(intervalo));
    }
    system("CLS"); // Limpiar la pantalla al final de la animación
    ...
    while (true) {
        ...
        switch (opcion) {
            ...
            case 3:
                system("CLS"); // Limpiar la pantalla
                animacion();
                std::cout << RESET; // Restablecer el color antes de imprimir el archivo
                std::cout << BLACK_TEXT_WHITE_BG; // Cambiar fondo a blanco y texto a negro
                copiar_e_imprimir_txt("ascii-art(3).txt"); // Copiar e imprimir el contenido del archivo
                std::cout << RESET; // Restablecer el color
                std::cout << WHITE_TEXT_BLACK_BG; // Cambiar fondo a negro y texto a blanco
                break;
            case 4:
                std::cout << RESET; // Restablecer el color
